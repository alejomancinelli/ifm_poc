...
def updatefig(*args):
    ...
    axAmp = imAmp.axes  # LOCAL FIX: AxesImage.get_axes() was removed in matplotlib 3.8
    ...
    axDist = imDist.axes  # LOCAL FIX: AxesImage.get_axes() was removed in matplotlib 3.8
...
def main():
    ...
    # LOCAL FIX: blit=True never repaints the titles this callback sets, and
    # cache_frame_data would grow without bound on an endless stream.
    ani = animation.FuncAnimation(fig, updatefig, interval=50, blit=False,
                                  cache_frame_data=False, fargs = [grabber,imAmplitude,imDistance])
    plt.show()
